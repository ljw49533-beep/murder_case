...
with tab2:
    ...
    if {'lat', 'lon', 'victim_race'}.issubset(filtered.columns) and not filtered.empty:
        fig_geo = px.scatter_mapbox(
            filtered,
            lat='lat', lon='lon',
            color='victim_race',
            hover_name='city',
            hover_data={'victim_race': True, 'victim_age': True, 'victim_sex': True, 'disposition': True},
            zoom=3, mapbox_style='carto-positron', size_max=8
        )
...
with tab3:
    ...
    with st.expander("모든 데이터(확장 시 전체 표시)", expanded=False):
